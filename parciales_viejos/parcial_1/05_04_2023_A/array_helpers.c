...
unsigned int best_profit(BakeryProductTable a)
{
    ...
    for (unsigned int city = 0; city < CITIES; city++){
        for (unsigned int season = 0; season < SEASONS; season++){
            /*Calculo la ganancia de cada ciudad y temporada en totalProfit*/
            totalProfit = (a[city][season].sale_value) - ((a[city][season].flour_cant * a[city][season].flour_price) + (a[city][season].yeast_cant * a[city][season].yeast_price) + (a[city][season].butter_cant * a[city][season].butter_price));

            /*Si la ganancia de la ciudad y temporada actual superan la anterior maxima ganancia registrada, entonces la maxima ganancia es actualizada.*/
            if (totalProfit > max_profit){
...
void array_from_file(BakeryProductTable array, const char* filepath)
{
    ...
    int i = 0;
    /*mientras no termine de leer el erchivo, continua el ciclo*/
    while (!feof(file)){
        season_t season;
        unsigned int city;
        ...
        /* La variable producto almacena todos los datos para cada ciudad y temporada y luego es cargada al array multidimensional en la posicion correspondiente. */
        BakeryProduct producto = bakery_product_from_file(file);
        array[city][season] = producto;

        ++i;
    }

   /* Aseguro antes de finalizar la funcion que la cantidad de lineas leidas del archivo sea exactamente igual a la cantidad total de ciudades, cada una con sus temporadas. */
    if (i!=CITIES*SEASONS){
